...
class CryptoSystem:
    def __init__(self, k: int, l: int, p: int = 0, q: int = 0):
        print("Rabin CryptoSystem starts")
        print("Storing k and l...")
        self.__k: int = k
        self.__l: int = l
        print("Generating keys...")
        keys = KeyGeneration(p, q)  # The keys are generated
        self._private_key = keys.generate_private_key()
        print(f"Private keys: {self._private_key}.")
        self._public_key = keys.generate_public_key()
        print(f"Public key: {self._public_key}.")

        self._alphabet = Alphabet()  # The alphabet is initialized
    ...
    @staticmethod
    def __jacobi_symbol(n: int, p: int) -> int:
        if p <= 0:
    ...
    def encrypt(self, plaintext: str) -> tuple[str, list[int]]:
        ...
        plaintext_redundancies = [self.__find_fitting_redundancy(len(self.__get_the_bit_field_of(numerical_equivalent)))
                                  for numerical_equivalent in plaintext_numerical_equivalents]
        ...
        for index in range(len(plaintext_numerical_equivalents)):
            plaintext_numerical_equivalents_with_redundancy.append(
                self.__get_m_with_the_replicated_last_k_bits(plaintext_numerical_equivalents[index],
                                                             plaintext_redundancies[index]))
    ...
    @staticmethod
    def __are_the_last_k_bits_replicated(m: int, k: int) -> bool:
        if k == 0:
            return True
        ...
        if len(m_as_bit_field) < 2 * k:
            raise Exception(
                f"There aren't enough bits in the message to check if the last {k} bits are replicated for message {m}.")
    ...
    def __find_acceptable_solution(self, solutions: list, redundancy: int, k: int) -> int:
        ...
        if len(acceptable_solutions) < 1:
            ...
            raise Exception(
                f"Too many acceptable solutions {acceptable_solutions} have been found for solutions {solutions}.")
    ...
    def decrypt(self, ciphertext: str, redundancies: list[int]) -> str:
        ...
        for index in range(len(decrypted_possible_numerical_equivalents)):
            decrypted_numerical_equivalents.append(
                self.__find_acceptable_solution(list(decrypted_possible_numerical_equivalents[index]),
                                                redundancies[index], k))
